  ...
  return (
    <>
      <Row gutter={16} style={{ marginBottom: 24 }}>
        <Col span={8}>
          <Card>
            <Statistic title="总市值" value={totalMarket} precision={2} prefix="¥" />
          </Card>
        </Col>
        <Col span={8}>
          <Card>
        ...
        <Col span={8}>
          <Card>
            <Statistic title="持仓数" value={positions.length} />
            <div style={{ marginTop: 16 }}>
              <Button type="primary" onClick={() => navigate('/investment/trades')}>
                录入交易
              </Button>
              <Button style={{ marginLeft: 8 }} onClick={() => navigate('/investment/positions')}>
                持仓列表
              </Button>
            </div>
          </Card>
      ...
      <Card title="资产分布（按标的）">
        ...
        {positions.length === 0 && (
          <p style={{ color: '#999' }}>暂无持仓，请先录入投资交易</p>
        )}
      </Card>
    </>
  );
