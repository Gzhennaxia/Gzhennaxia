...
面向个人的一体化数字中枢：在统一技术栈下逐步收纳**任务与日程、记账与理财、学习与考试（如题库/公考）**等模块，目标是「一个入口，管理多条人生主线」——包罗万象、按需生长，而非单一时间管理工具。

当前仓库已具备较完整的**任务与日历**能力，并包含**金融监控、题库、公考相关**等后端与前端雏形；其余领域（如记账流水、预算、资产全景等）可在该架构上持续扩展。
...
- **PostgreSQL 12+** - 主数据库（当前环境为 12，本地与服务器一致）
- **Caffeine 3.1.8** - 本地缓存
...
### 学习与职业拓展（已有代码/原型基础）
- ✅ 题库与课程相关服务与前端模块（`question-bank-service`、`QuestionBank`）
- ✅ 公考业务服务模块（`civil-servant-service`）及 `docs/civil-servant` 资料目录
- ✅ 理财与证券监控相关能力（`financial-service`、前端 `Finance`、原型与 IBKR 文档）

### 人生管理愿景（持续迭代）
- 记账：收支分类、账户、周期账单与对账（规划）
- 理财：持仓、收益、风险与目标（在现有金融模块上延伸）
- 习惯与健康、阅读与笔记等：可按同一「领域服务 + 前端路由」模式接入
...
### 计划功能（人生管理扩展）
- [ ] 记账：流水录入、分类、多账户、月报
- [ ] 理财：预算、目标、与现有监控/行情数据联动
- [ ] 统一「领域」导航与权限/数据隔离（个人单用户也可预留多配置档）
- [ ] 跨模块仪表盘（今日任务 + 财务快照 + 学习进度等）
